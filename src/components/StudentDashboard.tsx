...
export function StudentDashboard({ announcements, profile, onLogout }: StudentDashboardProps) {
  const fullName = [profile?.vardas, profile?.pavarde].filter(Boolean).join(' ').trim() || 'Mokinys';
  const classLabel = profile?.klase || 'Klasė nenurodyta';
      ...
      <header className="bg-[#3B2F2F] text-[#F5EFE6] px-4 py-4 shadow-lg sticky top-0 z-50">
        <div className="max-w-3xl mx-auto">
          ...
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }}>
            ...
            <p className="text-sm text-[#F5EFE6]/70">{classLabel}</p>
          </motion.div>
        </div>
      </header>
